...
export class VehicleTriviaPage {
  ...
  async ionViewWillEnter() {
    ...
    this.triviaApi.getVehicleTrivia().subscribe((data: any) => {
  ...
  async pickAnswer(answer: string) {
    ...
    if (answer === this.questions[this.currentIndex].correct_answer) {
      this.isCorrect = true;
      let vehicleTriviaCorrect = await this.storage.get('vehicle_trivia_correct');
      if (!vehicleTriviaCorrect) { vehicleTriviaCorrect = 0; }
      await this.storage.set('vehicle_trivia_correct', vehicleTriviaCorrect + 1);
      this.sessionCorrect++;
      ...
      let vehicleTriviaIncorrect = await this.storage.get('vehicle_trivia_incorrect');
      if (!vehicleTriviaIncorrect) { vehicleTriviaIncorrect = 0; }
      await this.storage.set('vehicle_trivia_incorrect', vehicleTriviaIncorrect + 1);
